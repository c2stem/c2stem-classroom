...
function generateDOT() {
    const blocks = JSON.parse(window.localStorage.getItem('blocks'));
    const treeRoots = JSON.parse(window.localStorage.getItem('treeRoots'));
    ...
    const connectNode = node => {
        for (let nextNode of node.next.contained) {
            if (nextNode) {
                ...
                const nextid = nextNode.id ? nextNode.id : `block_${String(Math.random()).slice(2)}`;
                if (!nextNode.id) {
                    dot += `${nextid} [label = <${nextNode.name ? nextNode.name : "no value"}>];\n`;
                }
